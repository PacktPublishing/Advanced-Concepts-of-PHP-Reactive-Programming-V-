...
require_once __DIR__ . '/../files/ProcessObservable.php';
...
class ServerManagerCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->loop->addReadStream($server, function() use ($server) {
            ...
            $server = new GameServerStreamEndpoint($client, $this->loop, $this->scheduler);
        ...
        foreach ($this->commands as $pattern => $method) {
            $stdin
                ...
                ->subscribe(function($value) use ($method) {
                    $this->$method($value);
                });
        }
        ...
        $this->statusSubject
            ->map(function() {
                ...
                return Observable::just(true, $this->scheduler)
            ...
            ->subscribe(function($statuses) use ($output) {
    ...
    private function spawnNewServer() {
        ...
        $process = new ProcessObservable($cmd, null, $this->scheduler);

        $this->output->writeln('Spawning new process on port ' . $port);

        $this->processes[$port] = $process->subscribe(
            null,
            function($e) use ($port) {
